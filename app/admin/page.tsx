...
// ── Action Button ──────────────────────────────────────────────────────────
function ActionBtn({ icon, label, color, onClick, danger, compact }: { icon: any; label: string; color: string; onClick: () => void; danger?: boolean; compact?: boolean }) {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .topbar-logo { height: 40px; width: auto; object-fit: contain; display: block; }
        ...
        <div style={{ background: IOH.white, borderBottom: `1px solid ${IOH.border}`, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto', padding: isMobile ? '0 14px' : '0 24px', height: isMobile ? 54 : 62, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 16 }}>
              <img src="/logo-ioh.png" alt="IOH" className="topbar-logo"
              ...
              <div style={{ display: 'none', alignItems: 'center', gap: 8, background: `linear-gradient(135deg, ${IOH.red} 0%, ${IOH.magenta} 100%)`, borderRadius: 8, padding: '5px 12px' }}>
                <ReceiptText size={14} color="#fff" /><span style={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>IOH</span>
              </div>
              ...
              <div>
                ...
                <div style={{ fontSize: 9, color: IOH.red, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 2 }}>Admin Panel</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: isMobile ? 6 : 8, alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', borderRadius: 8, border: `1.5px solid ${IOH.border}`, overflow: 'hidden' }}>
                <button onClick={() => setViewMode('list')} className={viewMode === 'list' ? 'view-btn-active' : ''} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: isMobile ? '7px 8px' : '7px 12px', border: 'none', background: IOH.white, cursor: 'pointer', fontSize: 12, fontWeight: 600, color: IOH.charcoal, fontFamily: "'Plus Jakarta Sans', sans-serif", transition: 'all 0.15s' }}>
                  <List size={13} /> {!isMobile && 'List'}
                </button>
                <button onClick={() => setViewMode('grid')} className={viewMode === 'grid' ? 'view-btn-active' : ''} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: isMobile ? '7px 8px' : '7px 12px', border: 'none', borderLeft: `1px solid ${IOH.border}`, background: IOH.white, cursor: 'pointer', fontSize: 12, fontWeight: 600, color: IOH.charcoal, fontFamily: "'Plus Jakarta Sans', sans-serif", transition: 'all 0.15s' }}>
                  <LayoutGrid size={13} /> {!isMobile && 'Grid'}
                </button>
              </div>
            ...
            {(!isMobile || filterOpen) && (
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(auto-fit, minmax(160px, 1fr))', gap: isMobile ? 10 : 14 }}>
                <div style={{ gridColumn: isMobile ? '1 / -1' : undefined }}>
                  ...
                  <select value={selectedDriver} onChange={e => setSelectedDriver(e.target.value)} style={{ ...inputStyle, fontSize: 14 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 5 : 6, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                      {(sub.amount ?? 0) > HIGH_VALUE_THRESHOLD && (
                        <div title={sub.proof_image_url ? 'Lihat bukti transfer' : 'Belum ada bukti transfer'} onClick={() => sub.proof_image_url && setLightbox(sub.proof_image_url)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: isMobile ? '5px 7px' : '5px 10px', borderRadius: 8, border: `1.5px solid ${sub.proof_image_url ? IOH.teal + '60' : '#FFE082'}`, background: sub.proof_image_url ? IOH.teal + '10' : '#FFFBEB', fontSize: 10, fontWeight: 700, color: sub.proof_image_url ? IOH.teal : '#D97706', cursor: sub.proof_image_url ? 'pointer' : 'default', whiteSpace: 'nowrap' }}>
                          {sub.proof_image_url ? <><CheckCircle2 size={10} /> {!isMobile && 'Bukti ✓'}</> : <>⚠ {!isMobile && 'Bukti'}</>}
                        </div>
                      )}
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 16 }}>
          ...
          <button onClick={() => setLightbox(null)} style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.12)', border: 'none', borderRadius: '50%', width: 40, height: 40, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
            <X size={18} color="#fff" />
          </button>
        </div>
      )}
