...
import ExportCSEOwnButton from '@/components/ExportCSEOwnButton'
import {
  LogOut, LayoutGrid, List, Camera, ImagePlus, Eye, EyeOff, X,
  ReceiptText, Car, Fuel, ParkingCircle, AlertTriangle, Pencil, Trash2,
  CheckCircle2, ChevronDown, Upload, AlertCircle, ZoomIn, MapPin
} from 'lucide-react'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: IOH.charcoal, lineHeight: 1.7, marginBottom: 24 }}>
          Nota <strong style={{ color: '#111' }}>{name}</strong> akan dihapus permanen dari sistem termasuk foto struk & foto timestamp-nya.
        </p>
...
export default function csePage() {
  ...
  // ── Target file yang lagi dipasangkan foto timestamp-nya ──
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(data => {
      ...
      if (data.user.role === 'driver') { router.replace('/driver'); return }
      setUser(data.user)
    })
  }, [])
  ...
  const removeFile = (id: string) => {
    setFiles(prev => {
      ...
      if (item) {
        ...
        if (item.markingPreview) URL.revokeObjectURL(item.markingPreview)
      }
      return prev.filter(f => f.id !== id)
    })
  }

  // ── Foto timestamp: dipasangkan per-nota lewat 1 hidden input yang di-retarget ──
  const openMarkingPicker = (id: string) => {
    setActiveMarkingId(id)
    setTimeout(() => markingRef.current?.click(), 0)
  }

  const handleMarkingFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    e.target.value = ''
    if (!file || !activeMarkingId) return
    setFiles(prev => prev.map(f => {
      if (f.id !== activeMarkingId) return f
      if (f.markingPreview) URL.revokeObjectURL(f.markingPreview)
      return { ...f, markingFile: file, markingPreview: URL.createObjectURL(file) }
    }))
  ...
  const removeMarking = (id: string) => {
    setFiles(prev => prev.map(f => {
      if (f.id !== id) return f
      if (f.markingPreview) URL.revokeObjectURL(f.markingPreview)
      return { ...f, markingFile: undefined, markingPreview: undefined }
    }))
  }

  const allHaveMarking = files.length > 0 && files.every(f => !!f.markingFile)
  const markingDoneCount = files.filter(f => f.markingFile).length

  const handleSubmit = async () => {
    if (!files.length || !allHaveMarking) return
    setUploading(true)
    setUploadResults(null)
    ...
    try {
      ...
      const compressedImages = await Promise.all(files.map(item => compressImage(item.file)))
      const compressedMarkings = await Promise.all(files.map(item => compressImage(item.markingFile as File)))

      compressedImages.forEach((file, i) => fd.append(`image_${i}`, file))
      compressedMarkings.forEach((file, i) => fd.append(`marking_${i}`, file))
  ...
  const filteredForView = needsProofFilter
    ...
    : incompleteFilter
    ? submissions.filter(s => getMissingFields(s).length > 0)
  ...
  const incompleteCount = submissions.filter(s => getMissingFields(s).length > 0).length
      ...
      <div style={{ minHeight: '100vh', background: IOH.bg, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div style={{ maxWidth: 520, margin: '0 auto', padding: '20px 16px 80px' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: '#111', letterSpacing: '-0.02em' }}>{user.name} <span style={{ fontSize: 11, fontWeight: 700, color: IOH.teal, background: IOH.teal + '15', padding: '2px 8px', borderRadius: 20, verticalAlign: 'middle' }}>cse</span></div>
          </div>
          ...
          <div style={{ background: IOH.white, borderRadius: 18, padding: 20, marginBottom: 20, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', border: `1px solid ${IOH.border}` }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#111', marginBottom: 4 }}>Upload Nota &amp; Foto timestamp</div>
            <div style={{ fontSize: 12, color: '#aaa', marginBottom: 18 }}>OCR otomatis baca nominal, kategori &amp; tanggal struk. Tiap nota wajib disertai foto timestamp.</div>
            {needsProofCount > 0 && (
              <div style={{
            ...
            {incompleteCount > 0 && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#92400E', lineHeight: 1.4 }}>
                    <strong>{incompleteCount} nota</strong> datanya belum lengkap
                  </span>
                </div>
                <button
                  onClick={() => { setPeriodFilter('all'); setNeedsProofFilter(false); setIncompleteFilter(true); setCurrentPage(1) }}
                  style={{
                    flexShrink: 0, padding: '7px 12px', borderRadius: 9,
                    border: '1.5px solid #F59E0B', background: '#F59E0B',
                    color: '#fff', fontSize: 11, fontWeight: 700,
            ...
            {files.length > 0 && (
              <>
                <div style={{
                  ...
                  background: allHaveMarking ? IOH.teal + '12' : '#FFFBEB',
                  border: `1.5px solid ${allHaveMarking ? IOH.teal + '40' : '#FFE082'}`,
                }}>
                  {allHaveMarking
                    ? <CheckCircle2 size={14} color={IOH.teal} />
                    : <MapPin size={14} color="#D97706" />
                  }
                  <span style={{ fontSize: 11, fontWeight: 700, color: allHaveMarking ? IOH.teal : '#92400E' }}>
                    {allHaveMarking
                      ? 'Semua nota sudah punya foto marki'
                      : `${markingDoneCount}/${files.length} nota sudah punya foto timestamp — wajib dilengkapi`}
                  </span>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14 }}>
                  {files.map(item => (
                    <div key={item.id} style={{ borderRadius: 10, overflow: 'hidden', background: IOH.border, border: `1.5px solid ${item.markingFile ? IOH.teal : IOH.border}` }}>
                      <div style={{ position: 'relative', aspectRatio: '3/4' }}>
                        ...
                        <div style={{ position: 'absolute', top: 5, left: 5, background: 'rgba(0,0,0,0.6)', color: '#fff', borderRadius: 5, padding: '1px 6px', fontSize: 9, fontWeight: 700 }}>Nota</div>
                      </div>

                      {/* ── Slot foto timestamp per item ── */}
                      {item.markingFile ? (
                        <div style={{ position: 'relative', aspectRatio: '3/1.4', background: '#f0f0f0' }}>
                          <img src={item.markingPreview} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="timestamp" />
                          <button onClick={() => removeMarking(item.id)} style={{ position: 'absolute', top: 3, right: 3, width: 18, height: 18, borderRadius: '50%', background: 'rgba(0,0,0,0.75)', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={9} /></button>
                          <div style={{ position: 'absolute', bottom: 2, left: 3, display: 'flex', alignItems: 'center', gap: 3, background: IOH.teal, borderRadius: 5, padding: '1px 5px' }}>
                            <MapPin size={8} color="#fff" />
                            <span style={{ fontSize: 8, fontWeight: 700, color: '#fff' }}>timestamp</span>
                          </div>
                        </div>
                      ) : (
                        <button onClick={() => openMarkingPicker(item.id)} style={{
                          width: '100%', aspectRatio: '3/1.4', border: 'none', borderTop: `1px dashed ${IOH.border}`,
                          background: '#FFFBEB', cursor: 'pointer', display: 'flex', flexDirection: 'column',
                          alignItems: 'center', justifyContent: 'center', gap: 2,
                          fontFamily: "'Plus Jakarta Sans', sans-serif",
                        }}>
                          <MapPin size={13} color="#D97706" />
                          <span style={{ fontSize: 8, fontWeight: 700, color: '#D97706' }}>+ Foto timestamp</span>
                        </button>
                      )}
                    </div>
                  ))}
            ...
            {files.length > 0 && (
              <button style={{
                width: '100%', padding: '13px 0', borderRadius: 12, border: 'none',
                background: uploading ? '#aaa' : !allHaveMarking ? IOH.border : `linear-gradient(135deg, ${IOH.red} 0%, ${IOH.magenta} 100%)`,
                color: !allHaveMarking && !uploading ? '#aaa' : '#fff',
                fontSize: 14, fontWeight: 700, cursor: uploading || !allHaveMarking ? 'not-allowed' : 'pointer',
                fontFamily: "'Plus Jakarta Sans', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                boxShadow: uploading || !allHaveMarking ? 'none' : '0 4px 16px rgba(237,28,36,0.35)',
              }} onClick={handleSubmit} disabled={uploading || !allHaveMarking}>
                {uploading
                  ...
                  : !allHaveMarking
                    ? <>Lengkapi Foto timestamp Dulu ({markingDoneCount}/{files.length})</>
                    : <>Upload {files.length} Nota ↑</>
                }
              </button>
            )}
          </div>
            ...
            <div className="fade-in" style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#111' }}>
                  {needsProofFilter ? '⚠️ Butuh Bukti Transfer' : incompleteFilter ? '⚠️ Data Belum Lengkap' : 'Riwayat Nota'}
                  {' '}<span style={{ color: '#bbb', fontSize: 13, fontWeight: 400 }}>({filteredForView.length})</span>
                </div>
                {(needsProofFilter || incompleteFilter) && (
                  <button onClick={() => { setNeedsProofFilter(false); setIncompleteFilter(false); setCurrentPage(1) }}
                    style={{
                      ...
                      return (
                        <div key={sub.id} style={{ borderRadius: 12, overflow: 'hidden', background: IOH.white, boxShadow: '0 2px 8px rgba(0,0,0,0.07)', border: `1px solid ${IOH.border}`, display: 'flex', flexDirection: 'column' }}>
                          <div style={{ position: 'relative', aspectRatio: '3/4', background: '#f0f0f0', cursor: 'zoom-in' }}
                            ...
                            {sub.marking_image_url && (
                              <div onClick={e => { e.stopPropagation(); setLightbox(sub.marking_image_url!) }} style={{ position: 'absolute', top: 5, right: 5, width: 20, height: 20, borderRadius: 6, overflow: 'hidden', border: '1.5px solid #fff', boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
                                <img src={sub.marking_image_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="timestamp" />
                              </div>
                            )}
                            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.72)', padding: '5px 7px' }}>
                              ...
                              {getMissingFields(sub).length > 0 && (
                                <div style={{
                                  ...
                                  <AlertTriangle size={10} /> Kosong: {getMissingFields(sub).join(', ')}
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <div key={sub.id} className="nota-card" style={{ background: IOH.white, borderRadius: 14, padding: '12px 14px', display: 'flex', gap: 12, alignItems: 'flex-start', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', border: `1px solid ${IOH.border}` }}>
                          {sub.image_url && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                              <img src={sub.image_url} style={{ width: 50, height: 62, objectFit: 'cover', borderRadius: 9, cursor: 'zoom-in', border: `1px solid ${IOH.border}` }} alt="nota" onClick={() => setLightbox(sub.image_url!)} />
                              {sub.marking_image_url ? (
                                <img src={sub.marking_image_url} style={{ width: 50, height: 30, objectFit: 'cover', borderRadius: 7, cursor: 'zoom-in', border: `1px solid ${IOH.teal}` }} alt="timestamp" onClick={() => setLightbox(sub.marking_image_url!)} />
                              ) : (
                                <div style={{ width: 50, height: 30, borderRadius: 7, border: `1px dashed ${IOH.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
